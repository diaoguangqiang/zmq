...
{	
	...
	{    
		...
		zmq_msg_recv(&recv_msg,z_socket, FLAG);
		size_t recv_size = zmq_msg_size(&recv_msg);
		if ( recv_size > 0 )
		{
			printf("server <<< size: %d, %s\n", /*recvn++*/zmq_msg_size(&recv_msg), (char*)zmq_msg_data(&recv_msg));
		}else{
			printf("server <<< no msg recv! line[%d]\n", __LINE__);
		}
		 //0表示非阻塞
		...
		zmq_sendmsg(z_socket,&send_msg, FLAG);
		printf("server >>> send success! msg[%s] line[%d]\n", msg_buf, __LINE__);
		//std::cout<<"server >>> msg: success! len "<< msg_len <<std::endl;
